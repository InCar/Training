...
tuple<int,int> Optica::SpatialTransfer(int x, int y, int width, int height, const SpatialDiff& delta)
{
	...
	while (fThetaHRaw >  +DirectX::XM_PI) fThetaHRaw -= static_cast<float>(DirectX::XM_PI + DirectX::XM_PI);
	while (fThetaHRaw <= -DirectX::XM_PI) fThetaHRaw += static_cast<float>(DirectX::XM_PI + DirectX::XM_PI);

	// 需要丢弃 (-Pi/2,+Pi/2)以外的部分
	float fU2, fV2;
	if (fThetaHRaw > -DirectX::XM_PIDIV2 && fThetaHRaw < DirectX::XM_PIDIV2)
		fU2 = fFocus * tanf(fThetaHRaw);
	else
		fU2 = 999999999.0f; // 设定一个很大的值,使其超出源图范围以外

	if (fThetaVRaw > -DirectX::XM_PIDIV2 && fThetaVRaw < DirectX::XM_PIDIV2)
